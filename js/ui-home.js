...
function loadHome() {
  // Class day = today matches the currently-selected/resolved session date
  // (master filter bar keeps AppState.filters.sessionId pointed at the
  // upcoming/current session — see _mfbReloadSessionOptions in ui-core.js).
  // NOT tied to a real-world day of the week — classes can be any day.
  const isClassDay = AppState.filters?.sessionId && AppState.filters.sessionId === getToday();

  // Class day: show Coordinator Performance only (hide leaderboard).
  // Other days: show leaderboard only (hide Coordinator Performance).
  document.getElementById('home-lb-podium-section')?.classList.toggle('hidden', isClassDay);
  document.getElementById('home-lb-table-section')?.classList.toggle('hidden', isClassDay);
  document.getElementById('home-coord-section')?.classList.toggle('hidden', !isClassDay);
...
async function renderHomeLeaderboard() {
  ...
  try {
    ...
    if (podiumEl) {
      // Only render real slots — no empty flex:1 spacer for a missing 3rd
      // place. With just a couple of departments (e.g. Male/Female) a phantom
      // spacer div would shift the real circles off-center instead of the
      // container's own justify-content:center doing its job.
      podiumEl.innerHTML = podiumOrder.map((team, pos) => ({ team, pos }))
        .filter(x => x.team)
        .map(({ team, pos }) => {
        ...
        <div class="table-scroll">
          <table class="lb-table">
            <thead><tr>
              ...
              <th class="lb-team-hdr">Department</th>${colHdrs}
...
async function renderTodaysActivity() {
  ...
  try {
    if (dayIdx === 6 && !filterSession) {
      ...
      if (link)  link.textContent  = 'Department Calling';
